...
function test_ibook_isObj(item: iBook) {
    console.log(
        `Is our iBook an object?: ${typeof item === 'object'}\n ${JSON.stringify(item, null, 4)}`
    );
...
const optional_entries_ibook: iBook = {
    title: 'test book 2',
    author: 'Ny Forfatter'
};
...
class Book implements iBook {
    ...
    set title(new_title: string) { this._title = new_title; }
    ...
    set published(new_pub: Date) { this._published = new_pub; }

    get pages() { return this._pages!; }
    set pages(new_pages: number) { this._pages = new_pages; }
}
...
class JavaBook implements iBook {
    ...
    published?: Date
    pages?: number
    constructor(private _title: string, private readonly _author: string, private _published?: Date, private _pages?: number) {
        this.title = _title;
        this.author = _author;
    ...
    setTitle(new_title: string) { this._title = new_title; }
    ...
    setPublished(new_pub: Date) { this._published = new_pub; }

    getPages() { return this._pages!; }
    setPages(new_pages: number) { this._pages = new_pages; }
}
